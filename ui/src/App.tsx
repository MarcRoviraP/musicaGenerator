...
export default function App() {
  ...
  const addPrompt = () => {
    setJobs(prev => [...prev, { id: Math.random().toString(36).substring(7), prompt: '', status: 'idle', subStatus: '' }]);
  };
  ...
  const runJob = async (job: Job) => {
    ...
    try {
      ...
      // 2. Create Account
      updateJobState(job.id, { subStatus: 'Creando email temporal...' });
      ...
      const createRes = await fetch(`${SERVER_BASE}/create`, {
        ...
        body: JSON.stringify({ prompt: job.prompt, style: 8 })
      });
      ...
      updateJobState(job.id, { status: 'error', error: err.message || 'Error desconocido', subStatus: '' });
    }
  };
